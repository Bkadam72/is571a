...
dept_employees(Es, Dept, DeptEmployees) :-
    [E | TailEmployees] = Es,
    employee(_Name, _Year, Dept, _Salary) = E,
    dept_employees(TailEmployees, Dept, TailResult),
    DeptEmployees = [E | TailResult].
dept_employees(Es, Dept, DeptEmployees) :-
    [E | TailEmployees] = Es,
    employee(_Name, _Year, OtherDept, _Salary) = E,
    OtherDept \= Dept,
    dept_employees(TailEmployees, Dept, DeptEmployees).
...
employees_salary_sum(_Es,_Sum).
employees_salary_sum([Zs,Es|Tail], Total) :-
employees_salary_sum([Zs+Es|Tail], Total).
...
% list_access([0 | Indexes], [H | _], Z) :-
%     list_access(Indexes, H, Z).
list_access([O | I], List, Z) :-
    nth0(O, List, Element),
    list_access(I, Element, Z).
list_access([O | _], List, Z) :-
    not(nth0(O, List, _)),
    Z = nil.
list_access([], List, Z) :-
    Z = List.
...
count_non_pairs(List, NNonPairs) :-
    not(atomic(List)),
    [Head | Tail] = List,
    count_non_pairs(Head, HeadNN),
    count_non_pairs(Tail, TailNN),
    NNonPairs is HeadNN + TailNN.
...
% L = Number to check divisibility for
divisible_by([H|T], L, Result) :-
    member(A, [H|T]),
   A mod L =:= 0,
    Result = A.
