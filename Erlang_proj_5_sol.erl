...
% functions.
dept_employees1(_, []) ->[];
dept_employees1(Dept, [Head|Tail]) ->
if 
Dept == Head#employee.dept ->[Head|dept_employees1(Dept, Tail)];
true ->dept_employees1(Dept, Tail)
end.
...
% [E] appended.
upsert_employee(E, []) -> [E];
upsert_employee(E1, [Head|Tail]) -> 
if 
E1#employee.name == Head#employee.name -> [E1|Tail];
true -> [Head|upsert_employee(E1,Tail)]
 end.
...
find_employees(Preds, Employees) ->[X || X <- Employees, lists:all(fun(P) -> P(X) end, Preds)].
...
employees_req(Req, Employees) ->
case Req of { delete, Name } -> { ok, void, delete_employee(Name, Employees) };
{ dump } -> { ok, Employees, Employees };
{ find, Preds } ->{ ok, find_employees(Preds, Employees), Employees };
{ read, Name } -> case find_employees([employee_has_name(Name)], Employees) of [Result] -> { ok, Result, Employees };
_ -> { err, "No employees found", Employees } end;
{ upsert, Employee } -> { ok, void, upsert_employee(Employee, Employees) };
_ -> { err, lists:flatten(io_lib:format("bad Req ~p", [Req])), Employees } end.
...
employees_req_with_sort(Req, Employees) -> 
case Req of {sort} -> SortedEmployees = lists:sort(fun(X1, X2) -> employee_name(X1) < employee_name(X2) end, Employees), {ok, void, SortedEmployees};        
_ -> employees_req(Req, Employees) end.
employee_name({employee, Name, _Age, _Department, _Salary}) -> Name.
...
start_employees_server(Employees, Fn) -> Server = spawn(fun () -> employees_server(Fn, Employees) end),
true = register(emps, Server), Server.
employees_server(Fn, Employees) -> receive {_Pid, {stop}} -> terminated;
{Pid, {new_fn, Fn1}} -> Pid ! {self(), {ok, void}}, employees_server(Fn1, Employees);
{Pid, Req} -> {Status, Result, Employees1} = Fn(Req, Employees), Pid ! {self(), {Status, Result}}, employees_server(Fn, Employees1)
end.
  
% stop previously started server with registered ID emps.
% should return {ok, stopped}.
stop_employees_server() -> emps ! {stop}, unregister(emps), {ok, stopped}.

% set request Req to server registered under ID emps and return 
% Result from server.
employees_client(Req) -> ServerPid = whereis(emps),
ServerPid ! {self(), Req}, receive
{ServerPid, {Status, Result}} -> {Status, Result}
end.
