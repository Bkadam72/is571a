...
deptEmployees [] _ = []
deptEmployees list d =
  if (emplDept (hs)) == d
  then (head list):(deptEmployees (tail list) d)
  else deptEmployees (ts) d
...
comprDeptEmployees employees dept =filter (\e -> emplDept e == dept) employees
...
employeesSalarySum employees = foldl (+) 0 (map emplSalary employees) 
...
expn :: (Floating a)  => a -> [a]
expn e = expnHelper e 0

expnHelper :: (Floating a)  => a -> Int -> [a]
expnHelper e x = element:(expnHelper e (x+1))
  where element = (e**(fromIntegral x))/(fromIntegral (factorial x))
...
charIndexes a b = [ xs | (x, xs) <- zip a [0..], x == b ]
...
foldTree treeF leafF tree =
  case tree of
    (Leaf t) -> leafF t
    (Tree t1 t t2) -> 
      treeFn (foldTree treeF leafF t1) t (foldTree treeF leafF t2)
...
flattenTree = foldTree (\hs x ts -> hs ++ x : ts) (\x -> [x])
